...
def test_vertical_bar():
    ...
    equivalent, msg = compare_dict(renderer.layout, VERTICAL_BAR['layout'])
    assert equivalent, msg
...
def test_horizontal_bar():
    ...
    for data_no, data_dict in enumerate(renderer.data):
        ...
        assert equivalent, msg
    equivalent, msg = compare_dict(renderer.layout, HORIZONTAL_BAR['layout'])
    assert equivalent, msg
...
def test_h_and_v_bars():
    ...
    for data_no, data_dict in enumerate(renderer.data):
        ...
        assert equivalent, msg
    equivalent, msg = compare_dict(renderer.layout, H_AND_V_BARS['layout'])
    assert equivalent, msg